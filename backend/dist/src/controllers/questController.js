...
exports.updateQuestCompletion = exports.generateQuests = exports.checkTodayStatus = void 0;
const prisma_1 = __importDefault(require("../db/prisma"));
// New endpoint to check if user has generated quests today
const checkTodayStatus = async (req, res) => {
    try {
        ...
        const existingDaily = await prisma_1.default.dailyQuest.findUnique({
            where: {
                userId_date: {
                    userId,
                    date: today,
        ...
        if (existingDaily) {
            ...
            const userMonthly = await prisma_1.default.monthlyQuest.findUnique({
                where: {
                    userId_month: {
                        userId,
            ...
            const sharedMonthly = await prisma_1.default.monthlyQuest.findFirst({
                where: {
                    month: currentMonthStr,
                },
            });
...
exports.checkTodayStatus = checkTodayStatus;
const generateQuests = async (req, res) => {
    try {
        ...
        const existingMonthly = await prisma_1.default.monthlyQuest.findFirst({
            where: {
                month: currentMonthStr,
            },
        });
...
Example format (GENERATE YOUR OWN based on user's feeling for personalized quests, general Canadian experiences for general Canadian quests):
{
  ...
  "journalPrompts": ["Personalized question about their feeling", "Deeper follow-up question", "Is there anything else you'd like to talk about today?"]
        ...
        questsData.journalPrompts = questsData.journalPrompts.map((prompt) => {
        ...
        questsData.quests = questsData.quests.map((q) => ({ ...q, completed: false }));
        questsData.monthlyQuests = questsData.monthlyQuests.map((q) => ({ ...q, completed: false }));
            ...
            if (!monthly) {
                ...
                monthly = await tx.monthlyQuest.create({
                    data: {
                        userId,
                        month: currentMonthStr,
                        monthlyQuests: existingMonthly.monthlyQuests, // Reuse the same quests
                    },
                });
            }
...
exports.generateQuests = generateQuests;
const updateQuestCompletion = async (req, res) => {
    try {
        ...
        if (isMonthly) {
            ...
            const monthlyQuest = await prisma_1.default.monthlyQuest.findFirst({
                where: {
                    userId,
                    month: currentMonthStr,
                },
            ...
            const monthlyQuests = monthlyQuest.monthlyQuests;
            ...
            await prisma_1.default.monthlyQuest.update({
            ...
            const dailyQuest = await prisma_1.default.dailyQuest.findUnique({
                where: {
                    userId_date: {
                        userId,
                        date: today,
...
exports.updateQuestCompletion = updateQuestCompletion;
